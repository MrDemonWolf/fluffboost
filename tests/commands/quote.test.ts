...
import { mockLogger, mockDb, mockDbChain, mockPosthog, mockClient, mockInteraction } from "../helpers.js";
...
describe("quote command", () => {
  ...
  async function loadModule() {
    ...
    return { execute: mod.execute, logger, db, posthog };
  }
  ...
  it("should capture posthog event on success", async () => {
    const { execute, db, posthog } = await loadModule();
    db.select.onCall(0).returns(mockDbChain([{ value: 1 }]));
    db.select.onCall(1).returns(mockDbChain([
      { id: "q1", quote: "Be brave", author: "Anon", addedBy: "user-1", createdAt: new Date() },
    ]));
    ...
    expect(posthog.capture.firstCall.args[0].event).toBe("quote command used");
  });
